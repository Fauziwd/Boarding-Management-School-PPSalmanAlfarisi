...
export default function AchievementIndex({ auth, achievements: initialAchievements }) {
    ...
    const handleDelete = async (id, santriName) => {
        try {
            const response = await axios.delete(route('achievements.destroy', id));
            if (response.data.success) {
                // Perbarui state untuk menghapus item dari daftar tanpa reload
                setAchievements(prevAchievements => ({
                    ...prevAchievements,
                    data: prevAchievements.data.filter(item => item.id !== id),
                    total: prevAchievements.total - 1
                }));
                // Tampilkan ToastComponent
                setToastMessage(`Pencapaian oleh ${santriName} berhasil dihapus.`);
            } else {
                throw new Error('Penghapusan gagal');
            }
        } catch (error) {
            console.error("Terjadi kesalahan saat menghapus data:", error);
            setToastMessage("Terjadi kesalahan, tidak dapat menghapus pencapaian.");
        }
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            ...
            <ToastComponent message={toastMessage} />  {/* Tempatkan ToastComponent di sini */}
            <div className="py-1">
                            ...
                            <table className="border-b-2 border-gray-200 min-w-full overflow-auto">
                                <thead>
                                    <tr className="border-b-2">
                                        ...
                                        <th className="px-6 py-3 text-left text-lg font-medium text-white">Tipe</th>
                                        <th className="px-6 py-3 text-left text-lg font-medium text-white">Title</th>
                                        <th className="px-6 py-3 text-left text-lg font-medium text-white">Deskripsi</th>
                                        <th className="px-6 py-3 text-left text-lg font-medium text-white">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {achievements.data.map(({ id, santri, type, title, description }, index) => (
                                        <tr key={id} className="border-b-2">
                                            ...
                                            <td className="px-6 py-4 text-lg text-white">{type}</td>
                                            <td className="px-6 py-4 text-lg text-white">{title}</td>
                                            <td className="px-6 py-4 text-lg text-white">{description}</td>
                                            <td className="px-6 py-4 text-lg text-white">
                                                <Link
                                                    href={route("achievements.edit", id)}
